...
from .document_extractor import DocumentExtractor, ManagedFolderDocumentRef, DocumentRef, InlineDocumentRef
...
from .semantic_model import DSSSemanticModel, DSSSemanticModelListItem
...
class DSSProject(object):
    ...
    def generate_ai_description(self, language="english", purpose="generic", length="medium", save_description=False):
        """
        ...
        :param str purpose: The purpose of the generated description. Supported purposes are "generic", "technical", "business_oriented", and "executive" (defaults to **generic**).
        :param str length: The length of the generated description. Supported lengths are "low", "medium", and "high" (defaults to **medium**).
        :param bool save_description: To save the generated description to this project (defaults to **False**).

        :returns: a message upon successful completion of the generated AI description. Only contains one `msg` field. For example, {'msg': 'An example description generated by AI'}
        :rtype: dict
        """
        supported_languages = {"dutch", "english", "french", "german", "japanese", "portuguese", "spanish"}
        supported_purposes = {"generic", "technical", "business_oriented", "executive"}
        supported_length = {"low", "medium", "high"}

        if language not in supported_languages:
            raise ValueError("Invalid language: '{}'. Currently supported languages: {}.".format(language, supported_languages))
        if purpose not in supported_purposes:
            raise ValueError("Invalid purpose: '{}'. Currently supported purposes: {}.".format(purpose, supported_purposes))
        if length not in supported_length:
            raise ValueError("Invalid length: '{}'. Currently supported lengths: {}.".format(length, supported_length))

        return self.client._perform_json("POST", "/projects/%s/generate-ai-description" % self.project_key, params={
            "language": language,
            "purpose": purpose,
            "length": length,
    ...
    def list_datasets(self, as_type="listitems", include_shared=False, tags=None):
        """
        ...
        items = self.client._perform_json(
            "GET",
            "/projects/%s/datasets/" % self.project_key,
            params={
                "foreign": include_shared,
                "tags": tags,
            },
        )
        if as_type == "listitems" or as_type == "listitem":
            ...
            return [
                DSSDataset(self.client, item["projectKey"], item["name"])
                for item in items
            ]
        else:
    ...
    def create_sample_dataset(self, dataset_name, plugin, sample, version=None):
        """
        ...
        :param str version: the version of the sample component inside the plugin, if unset, the preferred version of the sample will be created. Optional.
        ...
        if version is not None:
            obj["params"].update({"version": version})
    ...
    def create_finetuned_llm_saved_model(self, name):
        """
    ...
    def list_jupyter_notebooks(self, active=False, as_type="object"):
        ...
        :rtype: list of :class:`dataikuapi.dss.jupyternotebook.DSSJupyterNotebookListItem` or list of
            :class:`dataikuapi.dss.jupyternotebook.DSSJupyterNotebook`
        """
    ...
    def list_sql_notebooks(self, as_type="listitems"):
        """
        List the SQL notebooks of a project

        :param string as_type: How to return the list. Supported values are "listitems" and "objects" (defaults to
            **listitems**)
        ...
        :rtype: List of :class:`dataikuapi.dss.sqlnotebook.DSSSQLNotebookListItem` or list of
            :class:`dataikuapi.dss.sqlnotebook.DSSSQLNotebook`
        """
        ...
        if as_type == "listitems" or as_type == "listitem":
            ...
            return [DSSSQLNotebook(self.client, self.project_key, notebook_item["id"])
    ...
    def create_sql_notebook(self, notebook_content):
        ...
        notebook = self.client._perform_json("POST", "/projects/%s/sql-notebooks/" % self.project_key,
                                             body=notebook_content)
        return DSSSQLNotebook(self.client, self.project_key, notebook["id"])
    ...
    def render_document_template(self, template_type, data,
                                 template_str=None, template_bytes=None, template_ref=None,
                                 output_format=None, output_ref=None):
        """
        Render a document template

        :param str template_type: One of "CEL_EXPANSION", "JINJA", "DOCX_JINJA".
        :param dict data: Variables to substitute in the template.
        :param str template_str: Template content as string, supported by CEL_EXPANSION and JINJA.
        :param bytes template_bytes: Template as bytes, supported by DOCX_JINJA.
        :param DocumentRef template_ref: Input template document, a managed folder file or inline content.
        :param str output_format: Output format, one of "PDF", "DOCX", "TEXT".
        :param DocumentRef output_ref: Output document to write to, e.g. a managed folder file. When unspecified, return output inline.
        :returns: The rendered document
        :rtype: :class:`dataikuapi.dss.project.DocumentTemplateRenderingResponse`
        """
        valid_types = ["CEL_EXPANSION", "JINJA", "DOCX_JINJA"]
        if template_type not in valid_types:
            raise ValueError("Invalid template_type, it must be one of %s" % valid_types)
        if template_type == "DOCX_JINJA":
            if template_bytes is None and template_ref is None:
                raise ValueError("Either template_bytes or template_ref is required for DOCX_JINJA")
        else:
            if template_str is None and template_ref is None:
                raise ValueError("Either template_str or template_ref is required for %s" % template_type)

        valid_output_formats = ["PDF", "DOCX", "TEXT"]
        if output_format not in valid_output_formats:
            raise ValueError("Invalid output_format, it must be one of %s" % valid_output_formats)
        ...
        if template_ref is None:
            if template_str is not None:
                template_ref = InlineDocumentRef(template_str, InlineDocumentRef.CONTENT_TYPE_PLAIN_TEXT)
            elif template_bytes is not None:
                template_ref = InlineDocumentRef(base64.b64encode(template_bytes).decode("utf-8"), InlineDocumentRef.CONTENT_TYPE_BASE64_BYTES)
            else:
                raise ValueError("No template found, one of template_str, template_bytes or template_ref must be set")
        ...
        ret = self.client._perform_json(
            "POST",
            "/projects/%s/document-templating/actions/render-template" % self.project_key,
            body=payload)
        return DocumentTemplateRenderingResponse(self.client, self.project_key, ret)
    ...
    def export_bundle(self, bundle_id, release_notes=None, evaluate_project_standards_checks=True):
        """
        ...
        :param evaluate_project_standards_checks: indicates whether the Project Standards Checks applyting to this project should be run
        """
        return self.client._perform_json(
            method="PUT",
            path="/projects/%s/bundles/exported/%s" % (self.project_key, bundle_id),
            params={"releaseNotes": release_notes, "evaluateProjectStandardsChecks": evaluate_project_standards_checks}
        )
    ...
    def new_recipe(self, type, name=None):
        ...
        elif type == "nlp_agent_evaluation":
            return recipe.AgentEvaluationRecipeCreator(name, self)
        ...
        elif type == "nlp_llm_rag_embedding" or type == "embed_dataset":
            return recipe.EmbedDatasetRecipeCreator(name, self)
    ...
    def list_iceberg_namespaces(self, connection_name):
        """
        Lists namespaces from which tables can be imported in a Iceberg connection

        :param str connection_name: name of the Iceberg connection

        :returns: an array of namespaces
        :rtype: list
        """
        return self._list_namespaces(connection_name)
    ...
    def _list_namespaces(self, connection_name):
        return self.client._perform_json("GET", "/projects/%s/datasets/tables-import/actions/list-namespaces" % (
            self.project_key),
                                         params={"connectionName": connection_name})
    ...
    def list_iceberg_tables(self, connection_name, namespace=None):
        """
        Lists tables to import in a Iceberg connection

        :param str connection_name: name of the Iceberg connection
        :param str namespace: Optional, namespace in the Iceberg connection to list tables from.
        ...
        ret = self.client._perform_json("GET",
                                        "/projects/%s/datasets/tables-import/actions/list-tables" % (self.project_key),
                                        params={"connectionName": connection_name, "namespace": namespace})

        def to_namespace_table_pair(x):
            return {"namespace": x.get("namespace", None), "table": x["table"]}

        return [to_namespace_table_pair(x) for x in DSSFuture.get_result_wait_if_needed(self.client, ret)['tables']]
    ...
    def list_llms(self, purpose="GENERIC_COMPLETION", as_type="listitems"):
        ...
        :param str purpose: Usage purpose of the LLM. Main values are GENERIC_COMPLETION, TEXT_EMBEDDING_EXTRACTION, IMAGE_EMBEDDING_EXTRACTION, RERANKING and IMAGE_GENERATION
        :param str as_type: How to return the list. Supported values are "listitems" and "objects".
    ...
    def list_retrieval_augmented_llms(self, as_type="listitems"):
        """
        List the knowledge banks of this project

        :param str as_type: How to return the list. Supported values are "listitems" and "objects".
        :returns: The list of retrieval-augmented LLMs. If "as_type" is "listitems", each one as a :class:`dataikuapi.dss.retrieval_augmented_llm.DSSRetrievalAugmentedLLMListItem`.
                  If "as_type" is "objects", each one as a :class:`dataikuapi.dss.retrieval_augmented_llm.DSSRetrievalAugmentedLLM`
        :rtype: list[DSSRetrievalAugmentedLLM]
        """
        items = self.client._perform_json("GET", "/projects/%s/retrieval-augmented-llms" % (self.project_key))
        if as_type == "listitems":
            return [DSSRetrievalAugmentedLLMListItem(self.client, item) for item in items]
        elif as_type == "objects":
            return [DSSRetrievalAugmentedLLM(self.client, self.project_key, item["id"]) for item in items]
        else:
            raise ValueError("Unknown as_type")

    def get_retrieval_augmented_llm(self, id):
        """
        Get a handle to interact with a specific retrieval-augmented LLM

        :param id: the identifier of a retrieval-augmented LLM
        :returns: A :class:`dataikuapi.dss.retrieval_augmented_llm.DSSRetrievalAugmentedLLM` retrieval-augmented LLM handle
        """
        return DSSRetrievalAugmentedLLM(self.client, self.project_key, id)

    def create_retrieval_augmented_llm(self, name, knowledge_bank_ref, llm_id):
        """
        Create a new retrieval-augmented LLM in the project, and return a handle to interact with it

        :param str name: The name for the new retrieval-augmented LLM. This does not need to be unique
        :param str knowledge_bank_ref: Identifier of the Knowledge bank to use
        :param str llm_id: Identifier of the LLM to use for the RAG process

        :returns: a :class:`dataikuapi.dss.retrieval_augmented_llm.DSSRetrievalAugmentedLLM` handle to interact with the newly-created retrieval-augmented-llm
        """
        settings = {
            "name": name,
            "knowledgeBankRef": knowledge_bank_ref,
            "llmId": llm_id
        }

        ret = self.client._perform_json("POST", "/projects/%s/retrieval-augmented-llms" % self.project_key, body=settings)
        return DSSRetrievalAugmentedLLM(self.client, self.project_key, ret["id"])

    ########################################################
    # Semantic models
    ########################################################

    def list_semantic_models(self, as_type="listitems"):
        """
        List semantic models in this project.

        :param str as_type: How to return the list. Supported values are "listitems" and "objects".
        :returns: The list of semantic models. If "as_type" is "listitems", each one as a
                  :class:`dataikuapi.dss.semantic_model.DSSSemanticModelListItem`. If "as_type" is "objects",
                  each one as a :class:`dataikuapi.dss.semantic_model.DSSSemanticModel`
        :rtype: list
        """
        items = self.client._perform_json("GET", "/projects/%s/semantic-models/" % (self.project_key))
        if as_type == "listitems":
            return [DSSSemanticModelListItem(self.client, item) for item in items]
        elif as_type == "objects":
            return [DSSSemanticModel(self.client, self.project_key, item["id"]) for item in items]
        else:
            raise ValueError("Unknown as_type")
    ...
    def get_semantic_model(self, semantic_model_id):
        ...
        :param str semantic_model_id: identifier of a semantic model
        :returns: A :class:`dataikuapi.dss.semantic_model.DSSSemanticModel` handle
        """
        return DSSSemanticModel(self.client, self.project_key, semantic_model_id)

    def create_semantic_model(self, name):
        """
        Create a new semantic model in the project, and return a handle to interact with it.

        :param str name: The name for the new semantic model
        :returns: a :class:`dataikuapi.dss.semantic_model.DSSSemanticModel` handle
        """
        ret = self.client._perform_json(
            "POST", "/projects/%s/semantic-models/" % self.project_key, params={"name": name})
        return DSSSemanticModel(self.client, self.project_key, ret["id"])
    ...
    def list_agents(self, as_type="listitems"):
        """
        List the agents of this project

        :param str as_type: How to return the list. Supported values are "listitems" and "objects".
        :returns: The list of agents. If "as_type" is "listitems", each one as a :class:`dataikuapi.dss.agent.DSSAgentListItem`.
                  If "as_type" is "objects", each one as a :class:`dataikuapi.dss.agent.DSSAgent`
        :rtype: list[DSSAgent]
        """
        items = self.client._perform_json("GET", "/projects/%s/agents" % (self.project_key))
        if as_type == "listitems":
            return [DSSAgentListItem(self.client, item) for item in items]
        elif as_type == "objects":
            return [DSSAgent(self.client, self.project_key, item["id"]) for item in items]
        else:
            raise ValueError("Unknown as_type")
    ...
    def get_agent(self, id):
        ...
        :param id: the identifier of a agent
        :returns: A :class:`dataikuapi.dss.agent.DSSAgent` agent handle
        """
        return DSSAgent(self.client, self.project_key, id)

    def create_agent(self, name, type, plugin_agent_type=None):
        """
        ...
        settings = {
            "name": name,
            "type": type,
            "pluginAgentType": plugin_agent_type
        }
    ...
    def new_agent_tool(self, type, name=None, id=None):
        """Initiates the creation of an agent tool. Returns a :class:`dataikuapi.dss.agent_tool.DSSAgentToolCreator`
        or one of its subclasses to complete the creation of the agent_tool
        ...
        :param str type: Type of the agent tool to create
        :param str name: Optional, name of the new tool. Defaults to type if not specified
        :param str id: Optional, identifier of the new tool (must be unique). An id will be automatically assigned if not specified
        :rtype: :class:`dataikuapi.dss.agent_tool.DSSAgentToolCreator`
        """
        if name is None:
            name = type
        if type == "VectorStoreSearch":
            return agent_tool.DSSVectorStoreSearchAgentToolCreator(self, type, name, id)
        else:
    ...
    def list_plugins_usages(self):
        ...
        :returns: A list of :class:`DSSPluginUsagesListItem` objects
        :rtype: list
        """
        return [DSSPluginUsagesListItem(usages) for usages in self.client._perform_json("GET", "/projects/%s/plugins/usages" % self.project_key)]

    ################
    # Agent review
    ################
    def list_agent_reviews(self, as_type="listitems"):
        """
        ...
        :returns: The list of agent reviews. Each one is a handle to interact with the review.
        :rtype: if as_type=listitems, each review as a :class:`dataikuapi.dss.agent_review.DSSAgentReviewListItem`.
                if as_type=objects, each review is returned as a :class:`dataikuapi.dss.agent_review.DSSAgentReview`.
        """
        items = self.client._perform_json("GET", "/projects/%s/agent-reviews/reviews" % self.project_key)
        if as_type == "listitems":
            return [DSSAgentReviewListItem(self.client, self.project_key, item) for item in items]
        if as_type == "objects":
            return [DSSAgentReview(self.client, self.project_key, item) for item in items]

    def get_agent_review(self, agent_review_id):
        """
        Get a handle to interact with a specific agent review.

        :param str agent_review_id: The unique identifier of the agent review.
        :returns: A handle to interact with the specified agent review.
        :rtype: :class:`dataikuapi.dss.agent.DSSAgentReview`
        """
        review = self.client._perform_json("GET", "/projects/%s/agent-reviews/reviews/%s" % (self.project_key, agent_review_id))
        return DSSAgentReview(self.client, self.project_key, review)

    def create_agent_review(self, review_name):
        """
        Create a new agent review in this project.

        :param str review_name: The name for the new agent review. This does not need to be unique.
        :returns: A handle to interact with the newly created agent review.
        :rtype: :class:`dataikuapi.dss.agent.DSSAgentReview`
        """
        body = {
            "projectKey": self.project_key,
        ...
        ret = self.client._perform_json(
            "POST", "/projects/%s/agent-reviews/reviews" % self.project_key, body=body
        )
        return DSSAgentReview(self.client, self.project_key, ret)
...
class TablesImportDefinition(object):
    ...
    def add_iceberg_table(self, connection, namespace, table):
        """Add a Iceberg table to the list of tables to import

        :param str connection: the name of the Iceberg connection
        :param str namespace: the namespace of the table
...
class DSSProjectGit(object):
    ...
    def create_branch(self, branch_name, commit=None, duplicate_project=False, target_project_key=None, target_project_folder_id=None):
        """
        ...
        :param str commit: Optional. Hash of a commit to create the branch from.
        :param bool duplicate_project: Optional. If True (recommended), a new project is created as a duplicate with the new branch. Defaults to False.
        :param str target_project_key: Optional. Key of the target project where the branch should be created. If not provided and  `duplicate_project` is True,
                                    the new project key will default to the current project key appended with the branch name.
        :param str target_project_folder_id: Optional. The target project folder id to create the project into

        :return: A dict containing keys 'success' and 'output' with information about the command execution.
        :rtype: dict
        """
        return self.client._perform_json("POST", "/projects/%s/git/branches/" % self.project_key, body={"name": branch_name, "commit": commit, "duplicateProject": duplicate_project, "targetProjectKey": target_project_key, "targetProjectFolderId": target_project_folder_id})
    ...
    def reset_library(self, git_reference_path):
        """
        Reset the contents of the external library to match the specified
        Git reference (typically a branch or tag). It discards any local modifications.

        Internally it pulls the latest changes from the remote reference and override any local changes
...
class DocumentTemplateRenderingResponse(object):
    """
    Result of rendering a document template
    """
    def __init__(self, client, project_key, raw):
        """
        Do not instantiate directly, use :meth:`dataikuapi.dss.project.DSSProject.render_document_template`
        """
        self.client = client
        self.project_key = project_key
        self._raw = raw
    ...
    def get_raw(self):
        """
        :returns: The raw object returned by the rendering API.
        :rtype: dict
        """
        return self._raw
    ...
    def ok(self):
        """
        :returns: The outcome of the document templating rendering.
        :rtype: bool
        """
        return self._raw["ok"]
    ...
    def error(self):
        """
        :returns: The error if rendering failed.
        :rtype: str
        """
    ...
    def document(self):
        """
        :returns: The rendered template as a document reference.
        :rtype: :class:`DocumentRef`
        """
        if not self.ok:
            raise Exception("Cannot get output document from a failed rendering: %s" % self.error)
        if not "documentRef" in self._raw:
            raise Exception("No output document ref available in the rendering response")
        ...
        if document_ref.get("type") == "managed_folder":
            ...
            if not "content" in document_ref:
                raise Exception("No content available in the document ref")
            return InlineDocumentRef(
                document_ref["content"],
                document_ref.get("contentType", InlineDocumentRef.CONTENT_TYPE_PLAIN_TEXT),
                mime_type=document_ref.get("mimeType"))
        else:
            return document_ref
    ...
    def stream(self):
        """
        Get the rendered template as a binary stream.

        :returns: A file-like object with a read() method.
        :rtype: :class:`IOBase`
        """
        if not self.ok:
            raise Exception("Cannot stream document from a failed rendering: %s" % self.error)
        if not "documentRef" in self._raw:
            raise Exception("No output document ref available in the rendering response")
        ...
        elif isinstance(document, InlineDocumentRef):
            if document.is_base64():
                return io.BytesIO(document.as_bytes())
            else:
                return io.StringIO(document.as_string())
        else:
            raise Exception("Unsupported document type %s" % document.__class__.__name__)

    def text(self):
        """
        Get the rendered template as text. Only usable with plain text documents.

        :returns: Plain text document contents
        :rtype: str
        """
        content = self.stream()
        document = self.document
        if document.mime_type != "text/plain":
            raise Exception("text() is only available for plain text documents, use stream() instead")
        return content.read()
    ...
    def download_to_file(self, path):
        ...
        :param path: the path where to store the output
        :type path: str
        :returns: None
        """
        content = self.stream()
